...
### Changed
- `createDistribuerJournalpostRequest(...)` now requires a `dokumentProdApp` parameter, which is passed through to `DistribuerJournalpostRequest`.
...
### Changed
- Rename `PersonIdent` to `Personident`, and `personIdent` parameters to `personident`.
- Rename `NavIdent` to `Navident`, and `navIdent` parameters to `navident`.
